...
# YANGILANISH: db_update_step import qilindi
...
async def safe_send_message(bot: Bot, chat_id: int, *args, **kwargs):
    try: return await bot.send_message(chat_id, *args, **kwargs)
    except TelegramForbiddenError: pass
    except Exception as e: print(f"Xabar yuborishda xatolik {chat_id}: {e}")
    return None

async def clear_markup(bot: Bot, chat_id: int, message_id: int):
    if message_id:
        try: await bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id, reply_markup=None)
        except: pass

async def send_video_block(bot: Bot, chat_id: int, intro_text: str, video_data, footer_text: str, markup=None):
    if intro_text: await safe_send_message(bot, chat_id, text=intro_text, parse_mode="HTML")
    if isinstance(video_data, str):
        try: video_data = ast.literal_eval(video_data)
    ...
    for vid in video_ids:
        if vid:
            ...
            except Exception as e: print(f"Video xato (ID: {vid}): {e}")
...
async def send_nurture_msg(chat_id: int, day_num: int):
    ...
    try:
        if day_num == 1:
            ...
            try: 
                if config.NURTURE_VIDEO_2: await bot.copy_message(chat_id, config.CHANNEL_ID, config.NURTURE_VIDEO_2)
            except: pass
    ...
    except Exception as e: print(f"Nurture xatosi: {e}")
    finally: await bot.session.close()

# Auto funnel logic unchanged, skips re-updating step (we only track user intentional clicks)
...
def schedule_funnel_job(chat_id: int, step_name: str, delay_seconds: int, prev_msg_id: int):
    ...
    func_map = {'step_2': run_auto_step_2, 'case_1': run_auto_case_1, 'case_2': run_auto_case_2, 'case_3': run_auto_case_3, 'step_6': run_auto_step_6, 'step_7': run_auto_step_7}
    scheduler.add_job(func_map[step_name], 'date', run_date=run_date, args=[chat_id, prev_msg_id], id=job_id, replace_existing=True)

# ================= ASOSIY HANDLERLAR (Track qo'shilgan) =================
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    db_update_step(user_id, "0. Start bosildi") # Qadamni yozish
    ...
    for i, day_ahead in enumerate([config.NURTURE_DAY_1, config.NURTURE_DAY_2, config.NURTURE_DAY_3], 1):
        ...
        if run_date < datetime.datetime.now(tz): run_date += datetime.timedelta(days=1)
        scheduler.add_job(send_nurture_msg, 'date', run_date=run_date, args=[user_id, i], id=f"nurture_{user_id}_{i}")
...
async def process_step_2(callback: CallbackQuery):
    ...
    cancel_funnel(user_id)
    db_update_step(user_id, "1. 2-qadamga (Video) o'tdi")
...
async def process_case_1(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "2. Case 1 bosqichida")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_case_2(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "3. Case 2 bosqichida")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_case_3(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "4. Case 3 bosqichida")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_step_6(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "5. Demo videosida")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_not_now(callback: CallbackQuery):
    user_id = callback.from_user.id
    db_update_step(user_id, "Do'jim: Hozir emas bosdi")
...
async def process_buy_main(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id) 
    db_update_step(user_id, "💰 Sotib olishni bosdi")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_contact_admin(callback: CallbackQuery):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "📞 Admin bilan bog'lanish")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def process_fill_form(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    cancel_funnel(user_id)
    db_update_step(user_id, "📝 Anketa - Boshladi (1-savol)")
    await callback.message.edit_reply_markup(reply_markup=None)
...
async def form_niche(message: Message, state: FSMContext):
    user_id = message.from_user.id
    db_update_step(user_id, "📝 Anketa - Aylanmani kiritmoqda")
...
async def form_revenue(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    db_update_step(user_id, "📝 Anketa - Uchyot qismida")
...
async def form_accounting(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    db_update_step(user_id, "📝 Anketa - Raqam so'ralmoqda")
...
async def form_phone(message: Message, state: FSMContext):
    ...
    db_update_step(user_id, "✅ Anketa YAKUNLANDI (Lead keldi)") # Zayavka to'liq kiritildi
